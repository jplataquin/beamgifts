...
<head>
    ...
    <title>Print Vouchers - {{ $store->name }}</title>
    <!-- Include Bootstrap CSS for layout purposes before printing -->
    ...
    <div class="container-fluid py-4">
        ...
        <div class="d-flex justify-content-between align-items-start mb-4">
            <div>
                ...
                <h4 class="text-muted">Vouchers Report</h4>
            ...
            <tbody>
                @forelse($vouchers as $voucher)
                    <tr>
                        ...
                        <td>{{ $voucher->claimedBranch ? $voucher->claimedBranch->name : '-' }}</td>
                        <td>{{ $voucher->order->gifter->name ?? 'N/A' }}</td>
                        <td>{{ ucfirst($voucher->status) }}</td>
                        <td>{{ $voucher->status === 'claimed' && $voucher->claimedByUser ? $voucher->claimedByUser->name : '-' }}</td>
                        <td>{{ $voucher->created_at->format('M d, Y') }}</td>
                        <td class="text-end">₱{{ number_format($voucher->price ?? $voucher->product->price, 2) }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="7" class="text-center py-4">No vouchers found matching the criteria.</td>
                    </tr>
                @endforelse
            </tbody>
            <tfoot>
